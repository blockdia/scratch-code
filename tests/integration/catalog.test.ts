...
const auditSpec = (spec: BlockSpec, knownOpcodes: ReadonlySet<string>): AuditFailure[] => {
  ...
  spec.arguments.forEach((argument, index) => {
    ...
    if (!(argument.name in collection)) fail(`arguments.${index}.name`, `key in ${argument.kind}s`, argument.name)
  })
  for (const name of Object.keys(spec.inputs)) {
    if (!arguments_.has(`input:${name}`)) fail(`inputs.${name}`, "ordered argument reference", "missing")
  }
  for (const name of Object.keys(spec.fields)) {
    if (!arguments_.has(`field:${name}`)) fail(`fields.${name}`, "ordered argument reference", "missing")
  }
  ...
  for (const [name, field] of Object.entries(spec.fields)) {
    ...
    if (field.default !== undefined && (field.type === "variable" || field.type === "list" || field.type === "broadcast")) {
      const id = "id" in field.default ? field.default.id : undefined
      if (id !== undefined && typeof id !== "string") fail(`fields.${name}.default.id`, "string or omitted", id)
    }
  }

  const runtimeSpec = spec as unknown as {readonly hatStyle?: unknown; readonly outputType?: unknown}
  if (spec.shape === "hat" && (runtimeSpec.hatStyle !== "standard" && runtimeSpec.hatStyle !== "define")) {
    fail("hatStyle", "standard or define", runtimeSpec.hatStyle)
  }
  if (spec.shape !== "hat" && runtimeSpec.hatStyle !== undefined) fail("hatStyle", "omitted for non-hat", runtimeSpec.hatStyle)
  if (spec.shape === "reporter" && (typeof runtimeSpec.outputType !== "string" ||
...
describe("machine-readable TurboWarp catalog audit", () => {
  ...
  it("has unique scratchblocks identities or an explicit structural ambiguity strategy", () => {
    ...
    expect(ambiguous.map(([id, specs]) => ({id, opcodes: specs.map(spec => spec.opcode)}))).toEqual([
      {id: "CONTROL_IF", opcodes: ["control_if", "control_if_else"]},
    ])
  ...
  it("keeps resolver opcode and all resolved schema invariants", () => {
    ...
    const resolved = [
      ...
      registry.resolveRequired("procedures_call", {
        ...
        arguments: [{id: "n", type: "number"}, {id: "s", type: "string"}, {id: "b", type: "boolean"}],
      }),
    ...
    expect(resolved.map(spec => spec.opcode)).toEqual(["control_stop", "procedures_call", "procedures_prototype"])
    expect(failures, JSON.stringify(failures, null, 2)).toEqual([])
  })
})
