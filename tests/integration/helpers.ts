...
const inputProjection = (input: Input | ObscuredShadow): unknown => ({
  type: input.type,
  ...(input.type === "block" ? {value: blockProjection(input.value)}
    : input.type === "script" ? {value: scriptProjection(input.value)}
      : input.type === "empty" ? {} : {value: input.value}),
  ...(input.obscuredShadow === undefined ? {} : {obscuredShadow: inputProjection(input.obscuredShadow)}),
})

const blockProjection = (block: Block): unknown => ({
  opcode: block.opcode,
  ...(block.shadow === true ? {shadow: true} : {}),
  fields: Object.fromEntries(Object.entries(block.fields).map(([name, field]) => [name, {
    type: field.type,
    value: field.value,
    ...("id" in field && field.id !== undefined ? {id: field.id} : {}),
  }])),
  inputs: Object.fromEntries(Object.entries(block.inputs).map(([name, input]) => [name, inputProjection(input)])),
...
const surfaceInputProjection = (input: Input | ObscuredShadow): unknown => {
  ...
  if (input.type === "script") return {type: "script", value: surfaceProjection([input.value])[0]}
...
const surfaceBlockProjection = (block: Block): unknown => ({
  ...
  inputs: Object.fromEntries(Object.entries(block.inputs).map(([name, input]) => [name, surfaceInputProjection(input)])),
  ...(block.mutation === undefined ? {} : {mutation: block.mutation}),
})
