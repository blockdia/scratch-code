...
import { materializeDeterministically, semanticProjection, surfaceProjection } from './helpers.js';
...
describe('same-codec canonical round trips', () => {
  for (const fixture of wireFixtures) {
    ...
    it(`${fixture.name}: scratchblocks -> AST -> scratchblocks is stable`, () => {
      ...
      const secondAst = deserializeScratchblocks(
        parse(firstCanonical, { languages: ['en'] }),
        registry(),
      );
...
describe('cross-codec conversion matrix', () => {
  for (const fixture of wireFixtures) {
    ...
    it(`${fixture.name}: scratchblocks -> materialize -> VM Blocks and SB3`, () => {
      const source = deserializeScratchblocks(
        parse(fixture.scratchblocksText, { languages: ['en'] }),
        registry(),
      );
      const complete = materializeDeterministically(source);
      expect(
        semanticProjection(deserializeVmBlocks(serializeVmBlocks(complete), registry())),
      ).toEqual(semanticProjection(complete));
      expect(
        semanticProjection(deserializeSb3Blocks(serializeSb3Blocks(complete), registry())),
      ).toEqual(semanticProjection(complete));
    });
    ...
    it(`${fixture.name}: SB3 and VM Blocks -> scratchblocks`, () => {
      ...
      const sb3Surface = deserializeScratchblocks(
        serializeScratchblocks(sb3Ast, registry()),
        registry(),
      );
      const vmSurface = deserializeScratchblocks(
        serializeScratchblocks(vmAst, registry()),
        registry(),
      );
...
describe('shared semantic fixture oracle', () => {
  it('contains all 30 requested coverage categories', () => {
    ...
    expect(new Set(semanticFixtures.flatMap((fixture) => fixture.categories))).toEqual(
      new Set([
        'ordinary command stack',
        'number literal shadow',
        'string literal shadow',
        'color literal',
        'note',
        'matrix',
        'nested reporter',
        'reporter covers primitive shadow',
        'menu shadow',
        'reporter covers menu shadow',
        'Empty boolean input',
        'repeat',
        'SUBSTACK',
        'if',
        ...
        'procedure number/string/boolean arguments',
        'reporter procedure',
        'boolean reporter procedure',
        'top-level reporter',
        'disconnected scripts',
        'extension block',
        'comment',
        'scratchblocks diff/glow',
      ]),
    );
  });
  ...
  it('uses English as pivot and produces real Chinese output', () => {
    ...
    expect(
      deserializeScratchblocks(localized, registry()).map((script) => script.blocks[0]?.opcode),
    ).toEqual(['motion_movesteps']);
  });

  it('keeps materialize dynamic context callback on resolved base/final opcode', () => {
    const source = semanticFixtures
      .find((fixture) => fixture.name === 'procedure-call')!
      .createAst();
    const seen: string[] = [];
    let next = 1;
