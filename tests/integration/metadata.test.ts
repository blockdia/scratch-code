...
const metadataValues = (scripts: readonly Script[]): JsonValue[] => {
  ...
  for (const script of scripts) walk(script, {enter(node) {
...
describe("metadata namespace boundaries", () => {
  it("prevents private metadata from influencing peer codecs", () => {
    ...
    expect(serializeScratchblocks(foreign, createTurboWarpBlockRegistry()).stringify())
      .not.toBe(serializeScratchblocks(withoutScratchblocks, createTurboWarpBlockRegistry()).stringify())
    ...
    expect(serializeScratchblocks(foreign, createTurboWarpBlockRegistry()).stringify())
      .toBe(serializeScratchblocks(withoutForeignForScratchblocks, createTurboWarpBlockRegistry()).stringify())
  })

  it("keeps stable Scratch annotations small and node-specific", () => {
    const complete = materializeDeterministically(semanticFixtures.flatMap(fixture => fixture.createAst()))
    for (const metadata of metadataValues(complete)) {
      ...
      expect(Object.keys(scratch).every(key => ["id", "x", "y", "numericKind"].includes(key))).toBe(true)
    }
  })
