...
describe("production dependency DAG", () => {
  it("contains no cross-codec or inverted architecture edges", () => {
    const dependencies = Object.fromEntries(Object.entries(manifests).map(([name, manifest]) => [
      name,
    ...
    expect(dependencies).toEqual({
      ...
      "@scratch-code/scratchblocks-codec": [
        "@scratch-code/ast", "@scratch-code/block-spec", "scratchblocks-plus",
      ],
      "@scratch-code/turbowarp-blocks": ["@scratch-code/ast", "@scratch-code/block-spec"],
    })
    expect(dependencies["@scratch-code/scratchblocks-codec"]).not.toContain("@scratch-code/turbowarp-blocks")
  })
})
