...
describe("playground conversion", () => {
  ...
  it("exposes parsing and serialization separately for playground state", () => {
    ...
    expect(scriptsToText(scripts)).toBe(example.replace("when green flag clicked", "when flag clicked"))
  })
  ...
  it("reports a strict coercion mismatch", () => {
    expect(() => textToAst("move [not a number] steps :: motion", "strict"))
      .toThrow(/STEPS|number/i)
  })
  ...
  it("rejects a non-array AST root", () => {
    expect(() => astToText('{"kind":"script","blocks":[]}'))
      .toThrow("The AST must be a JSON array")
  })

  it("rejects malformed script and block nodes", () => {
    expect(() => assertScriptArray([{kind: "script", blocks: [{kind: "block"}]}]))
      .toThrow("opcode must be a non-empty string")
  })
})
