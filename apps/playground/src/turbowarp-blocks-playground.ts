import {TURBOWARP_BLOCKS_SOURCE_REVISION, turboWarpBlockSpecs} from "@scratch-code/turbowarp-blocks"
...
const searchable = (spec: BlockSpec): string => [spec.opcode, spec.shape, ...Object.keys(spec.inputs), ...Object.keys(spec.fields)].join(" ").toLowerCase()
const select = (spec: BlockSpec): void => {
  selected = spec.opcode
  output.textContent = JSON.stringify(spec, null, 2)
  for (const button of list.querySelectorAll("button")) button.classList.toggle("spec-button-active", button.dataset["opcode"] === selected)
}
const render = (): void => {
  ...
  list.replaceChildren(...matches.map(spec => {
  ...
  if (next === undefined) { selected = undefined; output.textContent = "No matching block specifications." }
  else select(next)
}
