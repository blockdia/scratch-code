...
const example: Script = {kind: "script", blocks: [{kind: "block", opcode: "motion_movesteps", fields: {}, inputs: {STEPS: {kind: "input", type: "number", value: "10"}}}]}
...
const analyze = (): void => {
  try {
    ...
    let maxDepth = 0
    walk(script, {enter(node, context) { counts[node.kind] += 1; maxDepth = Math.max(maxDepth, context.depth) }})
    window.ast = [script]
    output.textContent = JSON.stringify({counts, maxDepth, opcodes: script.blocks.map(block => block.opcode)}, null, 2)
    error.textContent = ""
...
document.querySelector("#reset")!.addEventListener("click", () => { reset(); analyze() })
reset(); analyze()
