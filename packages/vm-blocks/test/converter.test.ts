...
describe("VM block conversion", () => {
  it("round-trips stacks, scalar shadows, obscured shadows, comments, and coordinates", () => {
    const source: VmBlock[] = [{
      id: "hat", opcode: "event_whenflagclicked", next: "move", inputs: {}, fields: {},
      shadow: false, topLevel: true, x: 12, y: 34, comment: "comment-id",
    }, {
      id: "move", opcode: "motion_movesteps", next: "not", parent: "hat", fields: {}, shadow: false,
      inputs: {STEPS: {name: "STEPS", block: "variable", shadow: "number"}},
    }, {
      id: "variable", opcode: "data_variable", next: null, parent: "move", inputs: {}, shadow: false,
      fields: {VARIABLE: {name: "VARIABLE", value: "score", id: "variable-id", variableType: ""}},
    }, {
      id: "number", opcode: "math_number", next: null, parent: "move", inputs: {}, shadow: true,
      fields: {NUM: {name: "NUM", value: "10"}},
    }, {
      id: "not", opcode: "operator_not", next: null, parent: "move", fields: {}, shadow: false,
    ...
    expect(getVmBlocksBlockMetadata(scripts[0]!.blocks[0]!)).toEqual({version: 1, comment: "comment-id"})
    ...
    expect(JSON.stringify(getVmBlocksBlockMetadata(scripts[0]!.blocks[0]!))).not.toContain("opcode")
    ...
    expect(canonical["hat"]).toMatchObject({topLevel: true, parent: null, x: 12, y: 34, comment: "comment-id"})
    expect(canonical["move"]?.inputs?.["STEPS"]).toEqual({name: "STEPS", block: "variable", shadow: "number"})
    ...
    expect(canonical["variable"]?.fields?.["VARIABLE"]).toEqual({
      name: "VARIABLE", value: "score", id: "variable-id", variableType: "",
    })
  ...
  it("preserves raw mutations and JSON-valued historical fields", () => {
    const source: VmBlock[] = [{
      id: "stop", opcode: "control_stop", next: null, inputs: {}, shadow: false, topLevel: true,
  ...
  it("normalizes procedure mutations semantically", () => {
    const source: VmBlock[] = [{
      id: "call", opcode: "procedures_call", next: null, fields: {}, shadow: false, topLevel: true,
      inputs: {argument: {block: "text", shadow: "text"}},
      mutation: {
        tagName: "mutation", children: [], proccode: "say %s",
        argumentids: "[\"argument\"]", warp: false,
      },
    }, {
      id: "text", opcode: "text", next: null, parent: "call", inputs: {}, shadow: true,
  ...
  it("accepts omitted VM defaults and derives shadow ownership", () => {
    const source: VmBlock[] = [{
      id: "move", opcode: "motion_movesteps",
      inputs: {STEPS: {block: "number", shadow: "number"}},
    }, {
      id: "number", opcode: "math_number", fields: {NUM: {value: 5}},
    }]
    const scripts = deserializeVmBlocks(source, registry())
    expect(scripts[0]?.blocks[0]?.inputs["STEPS"]).toMatchObject({
      type: "number", value: 5, metadata: {scratch: {id: "number"}},
    })
    expect(byId(serializeVmBlocks(scripts))["number"]).toMatchObject({shadow: true, parent: "move"})
  })

  it("keeps disconnected VM shadow blocks marked as shadows", () => {
    const scripts = deserializeVmBlocks([{
      id: "argument", opcode: "argument_reporter_string_number", shadow: true,
  ...
  it("round-trips statement inputs, block shadows, lists, and broadcasts", () => {
    const source: VmBlock[] = [{
      id: "repeat", opcode: "control_repeat", topLevel: true,
      inputs: {
        TIMES: {block: "times", shadow: "times"},
        SUBSTACK: {block: "show", shadow: null},
      },
    }, {
      id: "times", opcode: "math_whole_number", parent: "repeat", shadow: true,
      fields: {NUM: {value: "3"}},
    }, {
      id: "show", opcode: "looks_show", parent: "repeat",
    }, {
      id: "key", opcode: "sensing_keypressed", topLevel: true,
      inputs: {KEY_OPTION: {block: "key-menu", shadow: "key-menu"}},
    }, {
      id: "key-menu", opcode: "sensing_keyoptions", parent: "key", shadow: true,
      fields: {KEY_OPTION: {value: "space"}},
    }, {
      id: "list", opcode: "data_listcontents", topLevel: true, x: 20, y: 30,
      fields: {LIST: {value: "items", id: "list-id", variableType: "list"}},
    }, {
      id: "broadcast", opcode: "event_whenbroadcastreceived", topLevel: true,
      fields: {
        BROADCAST_OPTION: {
          value: "go", id: "broadcast-id", variableType: "broadcast_msg",
        },
      },
    }]
  ...
  it("uses VM array order only to order roots", () => {
    ...
    expect(roots.map(script => script.blocks[0]?.metadata?.scratch?.id)).toEqual(["second", "first"])
  })
})
...
describe("VM block errors", () => {
  ...
  it("requires IDs for ordinary blocks and scalar shadows", () => {
    ...
    const missingShadow: Script[] = [{kind: "script", blocks: [{
      kind: "block", opcode: "looks_say", metadata: {scratch: {id: "say"}}, fields: {},
  ...
  it("rejects duplicate IDs across blocks and scalar shadows", () => {
    const scripts: Script[] = [{kind: "script", blocks: [{
      kind: "block", opcode: "looks_say", metadata: {scratch: {id: "same"}}, fields: {},
      inputs: {MESSAGE: {
        kind: "input", type: "string", value: "hello", metadata: {scratch: {id: "same"}},
      }},
    }]}]
    ...
    ], registry())).toThrow(DuplicateVmBlockIdError)
  })

  it("rejects invalid JSON, dangling references, cycles, and missing specs", () => {
    expect(() => deserializeVmBlocks([{
      id: "bad", opcode: "looks_say", fields: {VALUE: {value: Number.NaN}},
    }], registry())).toThrow(InvalidVmBlocksError)

    expect(() => deserializeVmBlocks([{
      id: "move", opcode: "motion_movesteps", inputs: {STEPS: {block: "missing", shadow: null}},
    }], registry())).toThrow(InvalidVmBlocksError)

    expect(() => deserializeVmBlocks([{
      id: "a", opcode: "looks_show", next: "b",
    }, {
      id: "b", opcode: "looks_hide", next: "a",
    }], registry())).toThrow(InvalidVmBlocksError)

    expect(() => deserializeVmBlocks([{id: "unknown", opcode: "extension_missing"}],
      createBlockSpecRegistry())).toThrow(MissingBlockSpecError)
  })

  it("rejects shared subgraphs and non-JSON AST data", () => {
    expect(() => deserializeVmBlocks([{
      id: "say", opcode: "looks_say", inputs: {MESSAGE: {block: "text", shadow: null}},
    }, {
      id: "think", opcode: "looks_think", inputs: {MESSAGE: {block: "text", shadow: null}},
    }, {
      id: "text", opcode: "text", fields: {TEXT: {value: "shared"}},
    }], registry())).toThrow(InvalidVmBlocksError)

    const invalidAst = [{kind: "script", blocks: [{
      kind: "block", opcode: "looks_show", metadata: {scratch: {id: "show"}},
      fields: {VALUE: {kind: "field", type: "text", value: undefined}}, inputs: {},
  ...
  it("rejects an explicit shadow flag which conflicts with its input edge", () => {
    expect(() => deserializeVmBlocks([{
      ...
      id: "number", opcode: "math_number", shadow: false,
      fields: {NUM: {value: "10"}},
    }], registry())).toThrowError(new InvalidVmBlocksError(
      'Block "number" has shadow false; expected true.',
    ))
  })
})
