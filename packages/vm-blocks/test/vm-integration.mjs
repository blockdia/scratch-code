...
const sourceTarget = vm.runtime.targets.find(
  (target) => Object.keys(target.blocks._blocks).length > 0,
);
...
assert.deepEqual(
  serializeVmBlocks(first),
  serialized,
  "shareBlocksToTarget must not mutate the caller's array",
);

vm.quit();
console.warn = originalWarn;
console.log(
  JSON.stringify({ revision, sourceBlocks: sourceBlocks.length, sharedBlocks: serialized.length }),
);
