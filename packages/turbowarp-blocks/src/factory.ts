...
const requireContext = <TContext extends Exclude<TurboWarpBlockResolveContext, undefined>>(
  ...
  if (context?.kind !== expectedKind) throw new InvalidTurboWarpBlockContextError(opcode, expectedKind)
...
const validCallContext = (context: ProcedureCallResolveContext): boolean =>
  ...
  context.arguments.every(argument =>
    typeof argument?.id === "string" && argumentTypes.has(argument.type))
...
const callInput = (type: TurboWarpProcedureArgumentType): InputSpec => {
  ...
  if (type === "number") return {connection: "value", accepts: "number", default: numericDefault(1, "number")}
  return {connection: "value", accepts: "string", default: {kind: "input", type: "string", value: ""}}
}
...
const resolveControlStop = (
  ...
  const stop = requireContext<{kind: "control-stop"; readonly hasNext: boolean}>(baseSpec.opcode, "control-stop", context)
  if (typeof stop.hasNext !== "boolean") throw new InvalidTurboWarpBlockContextError(baseSpec.opcode, "control-stop")
...
const resolveProcedureCall = (
  ...
  const call = requireContext<ProcedureCallResolveContext>(baseSpec.opcode, "procedure-call", context)
  if (!validCallContext(call)) throw new InvalidTurboWarpBlockContextError(baseSpec.opcode, "procedure-call")
  const inputs = Object.fromEntries(call.arguments.map(argument => [argument.id, callInput(argument.type)]))
  const common = {
    opcode: baseSpec.opcode,
    inputs,
    fields: baseSpec.fields,
...
const resolveProcedurePrototype = (
  ...
  const prototype = requireContext<ProcedurePrototypeResolveContext>(baseSpec.opcode, "procedure-prototype", context)
  if (!validPrototypeContext(prototype)) throw new InvalidTurboWarpBlockContextError(baseSpec.opcode, "procedure-prototype")
  const inputs = Object.fromEntries(prototype.arguments.map(argument => [argument.id, {
...
export const createTurboWarpBlockRegistry = (): BlockSpecRegistry<TurboWarpBlockResolveContext> => {
  ...
  for (const spec of turboWarpBlockSpecs) {
    ...
    else if (spec.opcode === "procedures_prototype") registry.register(spec, resolveProcedurePrototype)
