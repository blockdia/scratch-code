...
const defaults = toolboxDefaults as unknown as Readonly<
  Record<string, Readonly<Record<string, ShadowDefault>>>
>;
...
const sourceArguments = (json: SourceBlockJson | null): readonly SourceArgument[] =>
  json === null
    ? []
    : [json.args0, json.args1, json.args2, json.args3].flatMap(
        (argumentsForMessage) => argumentsForMessage ?? [],
      );
...
const literalDefault = (shadow: ShadowDefault): Input | undefined => {
  ...
  if (shadow.opcode === 'colour_picker')
    return { kind: 'input', type: 'color', value: value ?? '#7dcc8a' };
  if (shadow.opcode === 'matrix')
    return { kind: 'input', type: 'matrix', value: value ?? '0000000000000000000000000' };
...
const fieldBinding = (argument: SourceArgument): FieldSpecBindings => {
  ...
  const shape =
    type === 'field_colour' || type === 'field_colour_slider'
      ? ('color' as const)
  ...
  return {
    scratchblocks: { shape, ...(options === undefined || options.length === 0 ? {} : { options }) },
  };
};
const semanticFieldType = (opcode: string, argument: SourceArgument): FieldType => {
  ...
  if (
    rawFieldType(argument) === 'field_dropdown' ||
    rawFieldType(argument) === 'field_numberdropdown'
  )
    return 'dropdown';
  return 'text';
};
    ...
    sourceArguments(record.blockJson)
      .filter(
        (argument) =>
          ...
          argument.type?.startsWith('field_') === true &&
    ...
    value: {
      ...
      fields:
        record === undefined
          ? {}
          : Object.fromEntries(
              Object.entries(fieldsFor(record, shadow.fields)).map(([name, spec]) => [
                name,
                spec.default as Field,
              ]),
            ),
    },
  };
...
const acceptsFor = (
  ...
  if (opcode.startsWith('operator_')) {
    ...
    if (
      ['operator_join', 'operator_letter_of', 'operator_length', 'operator_contains'].includes(
        opcode,
      )
    ) {
...
const inputsFor = (record: SourceRecord): Readonly<Record<string, InputSpec>> =>
  Object.fromEntries(
    sourceArguments(record.blockJson)
      .filter(
        (argument) =>
          argument.name !== undefined &&
          (argument.type === 'input_value' || argument.type === 'input_statement'),
      )
      .map((argument) => {
        if (argument.type === 'input_statement')
          return [argument.name!, { connection: 'statement' }];
        const shadow = defaults[record.opcode]?.[argument.name!];
        const base = {
          connection: 'value' as const,
          accepts: acceptsFor(record.opcode, argument, shadow),
        };
        return [
          argument.name!,
          shadow === undefined ? base : { ...base, default: blockDefault(shadow) },
        ];
      }),
  );
...
const shapeFor = (record: SourceRecord): Pick<BlockSpec, 'shape'> & Partial<BlockSpec> => {
  ...
  if (record.opcode === 'procedures_call' || record.opcode === 'procedures_prototype')
    return { shape: 'command' };
  if (record.opcode === 'argument_reporter_string_number')
    return { shape: 'reporter', outputType: 'any' };
...
const argumentsFor = (record: SourceRecord): readonly BlockArgumentRef[] => {
  ...
  for (const argument of sourceArguments(record.blockJson)) {
    ...
    if (argument.type?.startsWith('field_') === true)
  ...
  if (
    record.opcode === 'procedures_definition' &&
    !arguments_.some((argument) => argument.name === 'custom_block')
  ) {
...
const createSpec = (record: SourceRecord): BlockSpec => {
  ...
  const inputs =
    record.opcode === 'procedures_definition'
      ...
      : extractedInputs;
  const fields =
    record.opcode === 'control_stop'
      ? {
          STOP_OPTION: {
            type: 'dropdown' as const,
            default: { kind: 'field' as const, type: 'dropdown' as const, value: 'all' },
          },
        }
      : fieldsFor(record);
