...
const inputDefault = (
  ...
  if (argument.type === "string") return {kind: "input", type: "string", value: argument.defaultValue ?? ""}
  if (argument.type === "color") {
    if (argument.defaultValue !== undefined) return {kind: "input", type: "color", value: argument.defaultValue}
    return {
      kind: "input",
      type: "block",
  ...
  if (argument.type === "matrix") {
    return {kind: "input", type: "matrix", value: argument.defaultValue ?? "0000000000000000000000000"}
  }
  if (argument.type === "note") return {kind: "input", type: "note", value: argument.defaultValue ?? "60"}
  ...
  if (argument.type === "sound") {
    ...
    return {
      kind: "input",
      type: "block",
      value: {kind: "block", opcode: "sound_sounds_menu", inputs: {}, fields: {SOUND_MENU: field}},
    }
  }
  return undefined
}
...
export const specsByExtensionId = new Map<string, readonly BlockSpec[]>(records.map(extension => [
  extension.id,
  [
    ...extension.blocks.map(block => createBlockSpec(extension, block)),
    ...extension.menus.filter(menu => menu.acceptReporters).map(menu => createMenuSpec(extension, menu)),
  ],
]))
