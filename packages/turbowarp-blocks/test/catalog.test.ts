...
describe("TurboWarp catalog", () => {
  it("contains all 294 audited serializable definitions", () => {
    const categories = [
      ...
      penBlockSpecs, wedo2BlockSpecs, musicBlockSpecs, microbitBlockSpecs,
    ...
    expect(flattened
      ...
      .map(spec => spec.opcode).sort()).toEqual(sourceManifest.map(record => record.opcode).sort())
    ...
    expect(flattened
      ...
      .map(spec => spec.opcode).sort()).toEqual(extensionOpcodes.sort())
  })
  ...
  it("preserves built-in extension inputs and menu serialization", () => {
    ...
    expect(registry.require("tw_getLastKeyPressed")).toMatchObject({
      shape: "reporter",
      outputType: "string",
      source: {scratchVm: {sourceFile: "src/extensions/tw/index.js", definition: "extension-info"}},
    })
