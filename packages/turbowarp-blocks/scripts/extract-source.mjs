...
export const SOURCE_FILES = [
  ...
  ...['math', 'text', 'colour', 'matrix', 'note'].map((name) => `blocks_common/${name}.js`),
];
...
export const extractSource = (scratchBlocksRoot) => {
  const sourceText = new Map(
    SOURCE_FILES.map((file) => [file, fs.readFileSync(path.join(scratchBlocksRoot, file), 'utf8')]),
  );
  const definitions = {};
  ...
  for (const [opcode, definition] of Object.entries(definitions)) {
    ...
    try {
      definition.init?.call(fakeBlock);
    } catch {
      /* custom block */
    }
    const file = SOURCE_FILES.find((candidate) =>
      sourceText.get(candidate).includes(`Blockly.Blocks['${opcode}']`),
    );
    records.push({ opcode, file, blockJson: blockJson ?? null });
  }
  vm.runInContext(
    fs.readFileSync(path.join(scratchBlocksRoot, 'blocks_vertical/default_toolbox.js'), 'utf8'),
    context,
  );
...
if (process.argv[1] !== undefined && pathToFileURL(process.argv[1]).href === import.meta.url) {
  process.stdout.write(
    `${JSON.stringify(extractSource(path.resolve(process.argv[2] ?? '')), null, 2)}\n`,
  );
}
