...
const expected = JSON.parse(
  fs.readFileSync(path.join(packageRoot, 'builtin-extensions-source-manifest.json'), 'utf8'),
);
assert.deepEqual(actual, expected);
const definitionCount = actual.reduce(
  (count, extension) =>
    count + extension.blocks.length + extension.menus.filter((menu) => menu.acceptReporters).length,
  0,
);
process.stdout.write(
  `${definitionCount} built-in extension definitions match scratch-vm ${TURBOWARP_VM_SOURCE_REVISION}\n`,
);
process.exit(0);
