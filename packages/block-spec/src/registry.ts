...
export class BlockSpecRegistry<TContext = unknown> {
  ...
  register(baseSpec: BlockSpec, resolver?: BlockSpecResolver<TContext>): this {
  ...
  replace(baseSpec: BlockSpec, resolver?: BlockSpecResolver<TContext>): this {
    if (!this.#entries.has(baseSpec.opcode)) {
...
): RegistryEntry<TContext> => (resolver === undefined ? { baseSpec } : { baseSpec, resolver });
