...
const unwrapScratchBlock = (block: ScratchBlock | ScratchGlow): {block: ScratchBlock; glow: boolean} => {
  if (block.isGlow) {
    if (!block.child.isBlock) {
      throw new InvalidScratchblocksAstError("A script glow cannot be used where a single block is required.")
    }
...
const resolveOpcode = <TContext>(state: DeserializeState<TContext>, block: ScratchBlock): string => {
...
const makeField = (
  ...
  const fallback = slot.spec.default === undefined
...
const sourceValueType = (input: ScratchInput): InputValueType => {
  ...
  if (input.isMatrix || (isRecord(input.value) && input.value["isMatrix"] === true)) return "matrix"
...
const matrixBits = (input: ScratchInput): string => {
  if (isRecord(input.value) && input.value["isMatrix"] === true && Array.isArray(input.value["rows"])) {
    return input.value["rows"].flat().map(value => value ? "1" : "0").join("")
  }
  ...
  switch (target) {
    case "number":
      return {
        ...
        ...(spec.default?.type === "number" && spec.default.metadata?.scratch?.numericKind !== undefined
    ...
    case "note":
      return {kind: "input", type: "note", value: typeof input.value === "number" ? input.value : String(input.value ?? "")}
    case "boolean":
...
const menuBlockFromInput = <TContext>(
  ...
  template.fields[firstSlot.name] = makeField(
    input,
    firstSlot,
    allLanguages["en"]!,
  )
...
const argumentReporter = (argument: ProcedureArgument): AstBlock => ({
  kind: "block",
  opcode: argument.type === "boolean" ? "argument_reporter_boolean" : "argument_reporter_string_number",
  inputs: {},
...
const convertProcedureDefinition = <TContext>(
  ...
  const procedureCode = block.info.call ?? ""
  const descriptor = state.procedures.get(procedureCode) ?? definitionDescriptor(
    block,
    path,
    state.createProcedureArgumentId,
  )!
  ...
  const prototype: AstBlock = {
    ...
    inputs: Object.fromEntries(descriptor.arguments.map(argument => [argument.id, {
      kind: "input",
      type: "block",
...
const convertValueChild = <TContext>(
  ...
  if (slot.spec.connection === "statement") {
    ...
    throw new ScratchblocksTypeMismatchError(opcode, slot.name, "a script", child.isInput ? `input ${child.shape}` : "a value block")
  }
...
const convertProcedureCall = <TContext>(
  ...
  const descriptor = state.procedures.get(block.info.call ?? "") ?? syntheticCallDescriptor(state, block, path)
  ...
  if (children.length > descriptor.arguments.length) {
    throw new InvalidScratchblocksAstError(`Procedure call "${descriptor.procedureCode}" has extra arguments.`)
  }
  const returnType: ProcedureCallMutation["returnType"] = block.isBoolean
    ? "boolean"
    : block.isReporter ? "reporter" : "statement"
  return {
    kind: "block",
...
const simpleReporter = <TContext>(
  state: DeserializeState<TContext>,
  block: ScratchBlock,
  opcode: "data_variable" | "data_listcontents" | "argument_reporter_boolean" | "argument_reporter_string_number",
): AstBlock => {
  const spec = state.registry.require(opcode)
  const fieldName = Object.keys(spec.fields)[0]
  if (fieldName === undefined) throw new InvalidScratchblocksAstError(`Reporter "${opcode}" has no field spec.`)
  const value = blockName(block) ?? block.children.flatMap(child => child.isLabel ? [child.value] : []).join(" ")
  return {
    kind: "block",
    opcode,
    inputs: {},
...
const convertScratchBlock = <TContext>(
  ...
  if (block.info.id === "PROCEDURES_DEFINITION") {
    ...
    if (opcode === "data_variable" || opcode === "data_listcontents" ||
        opcode === "argument_reporter_boolean" || opcode === "argument_reporter_string_number") {
      ...
      if (children.length > slots.length) {
        throw new InvalidScratchblocksAstError(`Block "${opcode}" has ${children.length - slots.length} extra structural children.`)
      }
      const inputs: Record<string, AstInput> = {}
      ...
      slots.forEach((slot, index) => {
        ...
        if (slot.kind === "field") {
          if (child !== undefined && !child.isInput) {
            throw new ScratchblocksTypeMismatchError(opcode, slot.name, "a field input", child.isScript ? "a script" : "a block")
          }
          fields[slot.name] = makeField(child?.isInput ? child : undefined, slot, allLanguages["en"]!)
        } else {
...
const englishLanguage = (): LanguageData => {
  ...
  if (english === undefined) throw new InvalidScratchblocksAstError("scratchblocks-plus did not load its built-in English language.")
  return english
}
...
const expectedScratchShape = (spec: InputSpec, input: AstInput): string => {
  ...
  if (accepts.includes("number") && !accepts.includes("string") && !accepts.includes("any")) return "number"
...
const scratchLiteralInput = (
  ...
  const accepts = acceptsList(spec)
  if (state.coercion === "strict" && actual !== undefined &&
      !accepts.includes("any") && !accepts.includes(actual)) {
  ...
  if (input.type === "matrix") {
    return new ScratchInputConstructor(shape, new ScratchMatrixConstructor(rowsFromMatrix(input.value)))
  }
...
const categoryFor = (spec: BlockSpec): string => {
  if (spec.opcode.startsWith("data_")) {
    return Object.values(spec.fields).some(field => field.type === "list") || spec.opcode.includes("list")
      ? "list"
      : "variables"
  }
...
const scratchShape = (spec: BlockSpec): string => {
  ...
  return Object.values(spec.inputs).some(input => input.connection === "statement") ? "c-block" : "stack"
}
...
const menuInputFromBlock = <TContext>(
  ...
  const field = block.fields[firstSlot.name] ?? (firstSlot.spec.default === undefined
    ? undefined
...
const serializeValueChild = <TContext>(
  ...
  if (input === undefined || input.type === "empty") {
    return spec.connection === "statement"
      ...
      : scratchLiteralInput(opcode, name, {kind: "input", type: "empty"}, spec, state as SerializeState<unknown>)
  }
  if (spec.connection === "statement") {
...
const prototypeFromDefinition = (block: AstBlock): AstBlock => {
  ...
  if (input?.type !== "block" || input.value.opcode !== "procedures_prototype") {
    throw new InvalidScratchblocksAstError("procedures_definition requires a procedures_prototype custom_block input.")
  }
  return input.value
}
...
const argumentTypeFromPrototype = (
  ...
  if (reporter?.type === "block" && reporter.value.opcode === "argument_reporter_boolean") return "boolean"
...
const serializeProcedureDefinition = <TContext>(
  ...
  if (prototype.mutation?.type !== "procedure-prototype") {
    throw new InvalidScratchblocksAstError("procedures_prototype requires a semantic prototype mutation.")
  }
  ...
  for (const part of procedureParts(mutation.proccode)) {
    if (/^%[nsb]$/.test(part)) {
      ...
      outlineChildren.push(customArgumentBlock(name, argumentTypeFromPrototype(prototype, mutation, argumentIndex)))
      argumentIndex += 1
    } else {
...
const serializeProcedureCall = <TContext>(
  ...
  for (const part of procedureParts(mutation.proccode)) {
    if (/^%[nsb]$/.test(part)) {
      ...
      children.push(serializeValueChild(
        state,
        block.opcode,
  ...
  const shape = mutation.returnType === "boolean" ? "boolean" : mutation.returnType === "reporter" ? "reporter" : "stack"
  const result = new ScratchBlockConstructor({
    id: "PROCEDURES_CALL",
    selector: "call",
    call: mutation.proccode,
...
const serializeOrdinaryBlock = <TContext>(
  ...
  const children: ScratchBlockChild[] = slots.map(slot => {
    if (slot.kind === "field") {
      const field = block.fields[slot.name] ?? (slot.spec.default === undefined
  ...
  result.translate(state.language)
  if (hasLoopArrow && !result.children.some(child => child.isIcon && child.name === "loopArrow")) {
    const scriptIndex = result.children.findIndex(child => child.isScript)
    result.children.splice(scriptIndex < 0 ? result.children.length : scriptIndex, 0, new ScratchIconConstructor("loopArrow"))
  }
  return result
}
...
const serializeAstBlock = <TContext>(
  ...
  if (block.opcode === "procedures_definition") scratchBlock = serializeProcedureDefinition(state, block)
      ...
      block.opcode === "argument_reporter_boolean" || block.opcode === "argument_reporter_string_number") {
