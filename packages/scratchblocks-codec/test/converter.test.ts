...
const semanticBlock = (block: Block): unknown => ({
  opcode: block.opcode,
  inputs: Object.fromEntries(Object.entries(block.inputs).map(([name, input]) => [name, semanticInput(input)])),
...
describe("ordinary Scratch blocks", () => {
  ...
  it("uses registry types for loose coercion and offers strict mode", () => {
    ...
    expect(() => deserializeScratchblocks(document, registry(), {coercion: "strict"}))
      .toThrow(ScratchblocksTypeMismatchError)
  })
  ...
  it("rebuilds labels through scratchblocks translate", () => {
    ...
    expect(serializeScratchblocks(scripts, registry(), {language}).stringify()).toBe("mover (10) pasos")
  })
...
describe("custom procedures and scratchblocks metadata", () => {
  ...
  it("allows callers to replace the deterministic procedure ID allocator", () => {
    const scripts = deserializeScratchblocks(parse("define p (value)", {languages: ["en"]}), registry(), {
...
describe("errors", () => {
  it("requires ordering metadata when a spec mixes multiple slots", () => {
    ...
    expect(() => serializeScratchblocks([script], customRegistry))
      .toThrow(MissingScratchblocksSpecMetadataError)
  })
})
