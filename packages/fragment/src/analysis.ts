...
const resourceKey = (reference: ResourceReference): string =>
  reference.id === undefined
    ? `value:${canonicalJson(reference.value)}`
    : `id:${reference.id}`
...
export const getTopProcedureDefinition = (
  script: Script,
): TopProcedureDefinition | undefined => {
  ...
  const concreteId = blockId !== undefined && blockId.length > 0
    ? blockId
    : prototypeId !== undefined && prototypeId.length > 0 ? prototypeId : undefined
  return {
    script,
    block,
    prototype,
...
export const analyzeScripts = (scripts: readonly Script[]): ScriptAnalysis => {
  ...
  const procedureDefinitions = collectTopProcedureDefinitions(scripts)
    .map(definition => cloneJson(definition.mutation))
