...
const call = (
  proccode: string,
  returnType: ProcedureReturnType = "statement",
): Block => ({
  kind: "block",
...
describe("analyzeScripts", () => {
  it("deduplicates resource references by semantic identity without creating IDs", () => {
    const scripts: Script[] = [ordinaryScript({
      kind: "block",
  ...
  it("reads definitions only from top blocks and finds calls recursively", () => {
    ...
    expect(analysis.procedureDefinitions.map(definition => definition.proccode))
      .toEqual(["defined"])
    expect(analysis.procedureCalls.map(procedureCall => [
    ...
    expect(analysis.unresolvedProcedureCalls.map(procedureCall => procedureCall.returnType))
      .toEqual(["reporter", "boolean"])
  })

  it("deduplicates exact calls while preserving reporter and boolean variants", () => {
    const scripts = [ordinaryScript(
      call("value", "reporter"),
      call("value", "reporter"),
      call("value", "boolean"),
    )]
    expect(analyzeScripts(scripts).procedureCalls.map(item => item.returnType))
      .toEqual(["reporter", "boolean"])
  })
  ...
  it("resolves statement, reporter, and boolean calls through one proccode identity", () => {
    ...
    expect(analysis.procedureCalls.map(item => item.returnType))
      .toEqual(["statement", "reporter", "boolean"])
    expect(analysis.unresolvedProcedureCalls).toEqual([])
  })

  it("reports known and unknown extension IDs without a registry", () => {
    const analysis = analyzeScripts([ordinaryScript(
...
describe("createScratchFragment", () => {
  it("adds the transitive closure in source order, not discovery order", () => {
    ...
    expect(fragment.scripts.map(script => analyzeScripts([script])
      .procedureDefinitions[0]?.proccode ?? "selected"))
  ...
  it("terminates direct and mutual recursion without duplicating definitions", () => {
    const direct = createScratchFragment(
      [ordinaryScript(call("A"))],
      {sourceScripts: [definitionScript("A", [call("A")], "a")]},
    )
    expect(direct.scripts).toHaveLength(2)

    const mutual = createScratchFragment(
      [ordinaryScript(call("A"))],
      {sourceScripts: [
  ...
  it("keeps transitive unresolved procedure dependencies minimal and unique", () => {
    const fragment = createScratchFragment(
      [ordinaryScript(call("A"))],
      {sourceScripts: [definitionScript("A", [
        call("external", "reporter"),
        call("external", "boolean"),
      ], "a")]},
    )
    expect(analyzeScripts(fragment.scripts).unresolvedProcedureCalls.map(item => item.returnType))
      .toEqual(["reporter", "boolean"])
  ...
  it("does not append a selected definition again", () => {
    ...
    expect(analyzeScripts(fragment.scripts).procedureDefinitions.map(item => item.proccode))
      .toEqual(["A", "B"])
  })
  ...
  it("rejects ambiguity in selected scripts or the reachable closure", () => {
    expect(() => createScratchFragment([
      definitionScript("A", [], "a-1"),
      definitionScript("A", [], "a-2"),
  ...
  it("rejects non-JSON AST values instead of normalizing them during cloning", () => {
    ...
    expect((selected[0]!.blocks[0]!.inputs["VALUE"] as {value: number}).value)
      .toBeNaN()
  })
})
