import type { AstNode, Block, Field, Input, ObscuredShadow, Script } from './types.js';
...
const invalidReplacement = (from: AstNode, to: unknown): never => {
  const replacementKind =
    typeof to === 'object' && to !== null && 'kind' in to ? String(to.kind) : typeof to;
  throw new TypeError(`Cannot replace ${from.kind} node with ${replacementKind} node`);
};
...
const transformBlockChildren = (
  ...
  return fields === block.fields && inputs === block.inputs ? block : { ...block, fields, inputs };
};
...
const transformInputChildren = (
  ...
  if (input.obscuredShadow !== undefined) {
    const obscuredShadow = transformNode(
...
function transformNode(node: Script, context: TransformContext, visitor: TransformVisitor): Script;
function transformNode(node: Block, context: TransformContext, visitor: TransformVisitor): Block;
function transformNode(node: Input, context: TransformContext, visitor: TransformVisitor): Input;
function transformNode(node: Field, context: TransformContext, visitor: TransformVisitor): Field;
function transformNode(
  ...
  if (typeof replacement !== 'object' || replacement === null || replacement.kind !== node.kind)
 ...
 */
export const transformScripts = (scripts: readonly Script[], visitor: TransformVisitor): Script[] =>
  scripts.map((script) =>
    transformNode(
      script,
      {
